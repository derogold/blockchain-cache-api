...
class DatabaseBackend {
  ...
  getLastBlockHeader () {
    return new Promise((resolve, reject) => {
      ...
      this.checkCache(cacheName)
        ...
        .then(() => {
          if (this.usePoolMonitor) return this.query('SELECT `b`.*, `p`.`id` AS `poolId`, `p`.`name` AS `poolName`, `p`.`url` AS `poolURL` FROM `blocks` AS `b` LEFT JOIN `blocks` AS `pb` ON `pb`.`hash` = `b`.`hash` LEFT JOIN `pools` AS `p` ON `p`.`id` = `pb`.`id` ORDER BY `b`.`height` DESC LIMIT 1', [])
  ...
  getBlockHeaderByHash (blockHash) {
    return new Promise((resolve, reject) => {
      ...
      this.checkCache(cacheName)
        ...
        .then(() => {
          if (this.usePoolMonitor) return this.query('SELECT `b`.*, `p`.`id` AS `poolId`, `p`.`name` AS `poolName`, `p`.`url` AS `poolURL` FROM `blocks` AS `b` LEFT JOIN `blocks` AS `pb` ON `pb`.`hash` = `b`.`hash` LEFT JOIN `pools` AS `p` ON `p`.`id` = `pb`.`id` WHERE `b`.`hash` = ? LIMIT 1', [blockHash])
  ...
  getBlockHeaderByHeight (height) {
    return new Promise((resolve, reject) => {
      ...
      this.checkCache(cacheName)
        ...
        .then(block => { topHeight = block.height })
        .then(() => {
          if (this.usePoolMonitor) return this.query('SELECT `b`.*, `p`.`id` AS `poolId`, `p`.`name` AS `poolName`, `p`.`url` AS `poolURL` FROM `blocks` AS `b` LEFT JOIN `blocks` AS `pb` ON `pb`.`hash` = `b`.`hash` LEFT JOIN `pools` AS `p` ON `p`.`id` = `pb`.`id` WHERE `b`.`height` = ? LIMIT 1', [height])
      ...
      this.checkCache(cacheName)
        ...
        .then(() => {
          return this.query([
            ...
            'LIMIT 288' // we hardcoded the 24h window to return stats to Explorer here. Avg. blocks produced in 24h based on block target.
          ].join(''), [])
        })
        .then(blocks => {
          if (blocks.length === 0) {
  ...
  getBlocks (height, count) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.checkCache(cacheName)
        ...
        .then(() => {
          return this.query([
            'SELECT `b`.`size`, `b`.`difficulty`, `b`.`hash`, `b`.`height`, `b`.`timestamp`, `b`.`nonce`, ',
      ...
      //    '`p`.`id` AS `poolId`, `p`.`name` AS `poolName`, `p`.`url` AS `poolURL` ',
            'FROM `blocks` AS `b` ',
      //    'LEFT JOIN `blocks` AS `pb` ON `pb`.`hash` = `b`.`hash` ',
      //    'LEFT JOIN `pools` AS `p` ON `p`.`id` = `pb`.`id` ',   //nedd to have a closer look at this, the join may not work due to not having pool monitor up
  ...
  getMixableAmounts(mixin) {
    mixin = (typeof mixin !== 'undefined') ? mixin : 2;
    ...
    return new Promise((resolve, reject) => {
      ...
      .then(cachedResults => resolve(cachedResults))
  ...
  getNodeStats () {
    return new Promise((resolve, reject) => {
      ...
      this.checkCache(cacheName)
        ...
        .then(() => { return this.query('SELECT `utcTimestamp` AS `stamp` FROM `node_polling` GROUP BY `utcTimestamp` ORDER BY `utcTimestamp` DESC LIMIT 20', []) })
        .then(rows => { rows.forEach(row => stamps.push(row.stamp)) })
/*        .then(() => {
          const query = util.format('SELECT `id`, ((SUM(`status`) / COUNT(*)) * 100) AS `availability` FROM `node_polling` GROUP BY `id`', stamps.join(','));

          return this.query(query, [])
*/
         .then(() => {
		    // Construct the query with WHERE clause using the stamps array
		     const stampsList = stamps.map(stamp => `'${stamp}'`).join(',');

		     //const stampsList = stamps.join(',');  // Convert stamps to a comma-separated string
 
		      // Ensure the stamps are used correctly in the query
		        const query = `
		            SELECT \`id\`, ((SUM(\`status\`) / COUNT(*)) * 100) AS \`availability\` 
		                FROM \`node_polling\` 
		                    WHERE \`utcTimestamp\` IN (${stampsList})
		                        GROUP BY \`id\`
		                          `;
        ...
        .then(() => { return this.query('SELECT MAX(`utcTimestamp`) AS `utcTimestamp` FROM `node_polling`', []) })
        .then(rows => {
          if (rows.length === 0) throw new Error('No timestamp information in the database')

          return this.query('SELECT * FROM `node_polling` WHERE `utcTimestamp` = ?', [rows[0].timestamp || 0])
        })
        .then(rows => {
          rows.forEach((row) => {
   ...
   /*     .then(() => {
          const query = util.format('SELECT `id`, `status`, `utcTimestamp` FROM `node_polling` ORDER BY `id` ASC, `utcTimestamp` DESC', stamps.join(','))

          return this.query(query, [])
   i*/
	  .then(() => {
		    const query = `
		      SELECT \`id\`, \`status\`, \`utcTimestamp\`
		          FROM \`node_polling\`
			      ORDER BY \`id\` ASC, \`utcTimestamp\` ASC
			        `;
